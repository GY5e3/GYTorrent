...
TorrentMetaData::TorrentMetaData(const std::string &filepath)
{
    ...
    {
        m_pieces.push_back(utils::Piece{m_pieceLength, bufferOffset, hashConcat.substr(index, 20)});
        bufferOffset += m_pieceLength;
    }
    m_pieces.push_back(utils::Piece{fullSize - bufferOffset, bufferOffset, hashConcat.substr(index, 20)});
...
std::vector<std::string> TorrentMetaData::GetTrackerURLs() const { return m_announceList; }
std::vector<utils::File> TorrentMetaData::GetFiles() const { return m_files; }
std::vector<utils::Piece> TorrentMetaData::GetPieces() const { return m_pieces; }
